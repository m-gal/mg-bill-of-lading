...
DROP TABLE IF EXISTS public.spst_cnee_carriers CASCADE;
CREATE TABLE public.spst_cnee_carriers AS
SELECT
    consignee_name,
    consignee_address,
    carrier_code,
...
CREATE INDEX ON public.spst_cnee_carriers(consignee_name, carrier_code, vessel_name, shipments_count);
CREATE INDEX ON public.spst_cnee_carriers(consignee_name, carrier_code, vessel_name, voyages_count);
CREATE INDEX ON public.spst_cnee_carriers(consignee_name, carrier_code, vessel_name, containers_count);
CREATE INDEX ON public.spst_cnee_carriers(consignee_name, carrier_code, vessel_name, value_sum);
CREATE INDEX ON public.spst_cnee_carriers(consignee_name, carrier_code, vessel_name, containers_count_avg);
CREATE INDEX ON public.spst_cnee_carriers(arrival_quarter_txt, consignee_name, carrier_code, vessel_name);